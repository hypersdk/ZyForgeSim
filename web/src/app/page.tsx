...
import { AppLink, Button, Card, Select, StatusBadge } from "@/components/ui";
    ...
    <div className="space-y-6">
      <Card title="Cluster Summary">
        <div className="flex flex-wrap items-end gap-3">
          <label className="text-sm text-hs-body">
            Config
            <Select className="ml-2" value={selected} onChange={(e) => setSelected(e.target.value)}>
              {configs.map((c) => (
      ...
      <Card title="Recent Runs">
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            ...
            <tbody>
              {runs.map((run) => (
                <tr key={run.id} className="border-t border-hs-border">
                  <td className="px-2 py-2 text-hs-heading">{run.config}</td>
                  <td className="px-2 py-2">
                    <StatusBadge status={run.status} />
                  </td>
                  <td className="px-2 py-2 font-mono text-xs text-hs-muted">{run.created_at.slice(0, 19)}</td>
      ...
      <Card title="Compare Two Configs">
        <div className="mb-3 flex flex-wrap gap-3">
          ...
          <Button variant="secondary" disabled={busy} onClick={handleCompare}>
            Compare
          </Button>
        </div>
